...
const incompleted = data.filter(function(unfinished) {
    return unfinished.completed == false;
});
console.log(incompleted);
// filter for user 6's tasks
const six = data.filter(function(currentUser) {
    return currentUser.userId == 6;
});
console.log(six);

// filter for tasks with a title less than 24 characters long
const titleLength = data.filter(task => task.title.length < 24);
